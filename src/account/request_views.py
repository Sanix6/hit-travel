...
from .models import RequestHotel, RequestTour
...
class TourRequestView(generics.CreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if tour_request_exists(tour_id, user):
            return Response(
                {"response": False, "message": "Заявка уже существует"}, status=400
            )
        ...
        if not lead_response:
            return Response(
                {"response": False, "message": "Ошибка создания лида"}, status=500
            )
        ...
        if not pdf_generated:
            return Response(
                {"response": False, "message": "Ошибка генерации PDF"}, status=500
            )

        transaction, deeplink = create_transaction(tour_request, serializer.data, user)
        if not transaction:
            return Response(
                {"response": False, "message": "Ошибка создания транзакции"}, status=500
            )

        bonuses_updated = decrease_user_bonuses(user, serializer.data["bonuses"])
        if not bonuses_updated:
            return Response(
                {"response": False, "message": "Ошибка уменьшения бонусов"}, status=500
            )
...
class RequestHotelView(views.APIView):
    ...
    def get(self, request, format=None):
        ...
        serializer = RequestHotelSerializer(
            user_requests, many=True, context={"type": "list"}
        )
        combined_data = []
...
class HotelDetail(views.APIView):
    ...
    def get(self, request, pk, format=None):
        ...
        serializer = RequestHotelSerializer(
            request_hotel, context={"type": "detail", "id": pk}
        )
        ...
        if response.status_code == 200:
            ...
            combined_data["user"] = PersonalInfoSerializer(
                request_hotel.user, context={"request": request}
            ).data

            return Response(combined_data)
        else:
            return Response(
                {"error": "Ошибка при получении данных отеля"},
