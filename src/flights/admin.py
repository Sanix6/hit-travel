...
from .models import AviaAgreement, AirProviders, Segments, Passengers, FlightRequest, FlightCancel
...
class FlightRequestAdmin(admin.ModelAdmin):
    ...
    list_display = ['user',  'get_from_to', 'book_class', 'segment_type', 'amount', 'paid', 'status', 'created_at']
    list_display_links = list_display
    ordering = ["-created_at"]
    ...
    def get_from_to(self, obj):
        if obj.segments.exists():
            ...
            if first_segment and last_segment:
                ...
                if (first_segment.from_iata == last_segment.to_iata and num_segments == 2) or \
    ...
    def segment_type(self, obj):
        if obj.segments.exists():
            ...
            if obj.segments.first().from_iata == obj.segments.last().to_iata and num_segments == 2:
                return f"Туда и обратно"

            if obj.segments.first().from_iata == obj.segments.last().to_iata and num_segments > 2:
                return f"Туда и обратно с пересадкой"
            
            if obj.segments.first().from_iata != obj.segments.last().to_iata and num_segments > 1:
