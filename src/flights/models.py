...
class FlightRequest(models.Model):
    ...
    client_email = models.CharField(
        _("Почта клиента"), max_length=150, blank=True, null=True
    )
    client_phone = models.CharField(
        _("Телефон клиента"), max_length=150, blank=True, null=True
    )
    payer_name = models.CharField(
        _("Имя покупателя"), max_length=150, blank=True, null=True
    )
    billing_number = models.CharField(
        _("Номер брони"), max_length=500, blank=True, null=True
    )
    ...
    partner_affiliate_fee = models.DecimalField(
        _("Комиссия партнера"), max_digits=10, decimal_places=2, null=True, blank=True
    )
...
class Passengers(models.Model):
    main = models.ForeignKey(
        FlightRequest, on_delete=models.CASCADE, related_name="passengers"
    )
    ...
    age = models.CharField(
        _("Тип возраста"), choices=PASSENGER_TYPE, max_length=150, blank=True, null=True
    )
    birthdate = models.DateField(_("Дата рождения"), blank=True, null=True)
    doctype = models.CharField(
        _("Тип документа"), choices=DOCUMENT_TYPE, max_length=5, blank=True, null=True
    )
    docnum = models.CharField(
        _("Номер документа"),
        max_length=50,
        help_text="Серия и номер",
        blank=True,
        null=True,
    )
    docexp = models.DateField(_("Действует до"), blank=True, null=True)
    gender = models.CharField(
        _("Пол"), choices=PASSENGER_GENDER, max_length=5, blank=True, null=True
    )
    citizen = models.CharField(
        _("Код страны"),
        max_length=5,
        help_text="код страны в формате ISO 3166-1 alpha-2",
        blank=True,
        null=True,
    )
    phone = models.CharField(
        _("Телефон номер пассажира"), max_length=100, blank=True, null=True
    )
    email = models.CharField(
        _("Эл-почта пассажира"), max_length=100, blank=True, null=True
    )
...
class Segments(models.Model):
    main = models.ForeignKey(
        FlightRequest, on_delete=models.CASCADE, related_name="segments"
    )

    from_name = models.CharField(
        _("Откуда (город)"), help_text="Алматы", max_length=150
    )
    from_iata = models.CharField(_("Код города"), help_text="ALA", max_length=150)
    from_country = models.CharField(
        _("Страна отправления"), help_text="Казахстан", max_length=150
    )
    ...
    to_country = models.CharField(
        _("Страна прибытие"), help_text="Кыргызстан", max_length=150
    )
...
class FlightCancel(models.Model):
    transaction = models.ForeignKey(
        to=payment_model.Transaction,
        on_delete=models.CASCADE,
        verbose_name=_("Транзакция"),
        related_name="flight_cancel",
    )
    flight = models.ForeignKey(
        to=FlightRequest, on_delete=models.CASCADE, verbose_name=_("Билет")
    )
