...
class MPaymentView(APIView):
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            if qm != "MOB":
                return self.error_response(datetime_now, qid, "401", "Некорректное значение QM")
            ...
            if op == "QE10":  # ConfirmPaymentView
                ...
                return self.error_response(datetime_now, qid, "401", "Некорректное значение OP")

        except ParseError:
            return self.error_response(datetime_now, None, "401", "Некорректный формат данных")

        except Exception as e:
            logging.error(e)
            return self.error_response(datetime_now, None, "424", "Сервис временно недоступен")
    ...
    def confirm_payment(self, xml_body, xml_head, datetime_now):
        ...
        try:
            ...
            if transaction.amount == float(xml_body.get("SUM")) and transaction.status == "processing":
                ...
                return self.error_response(datetime_now, qid, "423", "Заявленная сумма платежа меньше стоимости услуги")

            elif transaction.amount < float(xml_body.get("SUM")):
                return self.error_response(datetime_now, qid, "422", "Заявленная сумма платежа превышает стоимость услуги")

        except Transaction.DoesNotExist:
            return self.error_response(datetime_now, qid, "420", "Лицевой счет не найден")

    def check_payment(self, xml_body, xml_head, datetime_now):
        qid = xml_head.get("QID")
        try:
            if Transaction.objects.filter(qid=qid).exists():
                return self.error_response(datetime_now, qid, "421", "Дублирование платежа")
...
class PaylerPaymentView(APIView):
    ...
    def post(self, request):
        ...
        if serializer.is_valid():
            try:
                transaction = Transaction.objects.get(id=serializer.data.get("order_id"), user=request.user, status="processing")
            except Exception as e:
            ...
            if response.status_code != 200:
                return Response({"response": False, "message": response_data["error"]["message"]})
...
class PaylerCallbackView(APIView):
    def post(self, request):
        ...
        try:
            ...
            return Response({"response": True, "message": "Не удалось найти счет."}, status=404)
        except Exception as e:
        ...
        if response.status_code != 200:
            return Response({"response": True, "message": f'{response.json()["error"]["message"]}'})
        ...
        if response.get("status") == "Charged":
            if transaction.name == "ticket":
                ...
                return Response({"response": False, "message": "Invalid transaction name."}, status=400)

            transaction.status = "completed"
            transaction.save()

            return Response({"response": True, "message": "Продукт куплен, можете перейти на приложение)"})
        else:
